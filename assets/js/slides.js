...
var Slides = (function($) {
    ...
    function init() {
        var head = $("head").html();
        CSS_FONTS = /"([\S]*fonts[\S]*\.css[\S]*)"/.exec(head)[1];
        CSS_SLIDES = /"([\S]*slides[\S]*\.css[\S]*)"/.exec(head)[1];
        CSS_STYLE = /"([\S]*style[\S]*\.css[\S]*)"/.exec(head)[1];
        CSS_ANIMATIONS = /"([\S]*animations[\S]*\.css[\S]*)"/.exec(head)[1];
        CSS_FONTAWESOME = /"([\S]*font-awesome[\S]*\.css[\S]*)"/.exec(head)[1];
        JS_MATHJAX = /"([\S]*MathJax[\S]*\.js[\S]*)"/.exec(head)[1];
        setupProgressbar();
        setupControls();
        setupHelp();
        setupSlides();
