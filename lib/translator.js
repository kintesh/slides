...
;(function(){
    ...
    var constants = require("./constants"),
        ...
        async = require("async"),
        katex = require("katex"),
        mathjax = require("MathJax-Node").single;
    ...
    function extractMaths(slides, callback) {
        ...
        try {
            slides.frames.map(function(frame) {
    ...
    function restoreMaths(slides, callback) {
        ...
        try {
            slides.frames.map(function(frame) {
                for(var i=0; i<frame.maths.length; i++) {
                    if(frame.maths[i].type == constants.math_type_inline) {
                        frame.content = frame.content.replace(constants.math_marker(i),
                            constants.temp_inlineMath(frame.maths[i].value))
                    }
                    if(frame.maths[i].type == constants.math_type_block) {
                        frame.content = frame.content.replace(constants.math_marker(i),
                            constants.temp_blockMath(frame.maths[i].value))
                    }
                }
            });
            callback(null, slides);
    ...
    function makeTitleSlide(slides, callback) {
        ...
        try {
            ...
            titleSlide = titleSlide != "" ? constants.temp_slide(titleSlide) : undefined;
    ...
    /**
     * Renders maths using KaTeX
     */
    function renderKatex(slides, callback) {
        checkCallback(callback);
        try {
            slides.frames.map(function(frame) {
                for(var i=0; i<frame.maths.length; i++) {
                    if(frame.maths[i].type == constants.math_type_inline) {
                        frame.content = frame.content.replace(constants.math_marker(i),
                            katex.renderToString(frame.maths[i].value));
                    } else if(frame.maths[i].type == constants.math_type_block) {
                        frame.content = frame.content.replace(constants.math_marker(i),
                            katex.renderToString("\\displaystyle {" + frame.maths[i].value  + "}"));
                    }
                }
            });
            callback(null, slides);
    ...
    /**
     * Renders maths using MathJax
     */
    function renderMathJax(slides, callback) {
        checkCallback(callback);
        mathjax.config({MathJax: {menuSettings: {semantics: false, texHints: false}}});
        try {
            async.map(slides.frames, function(frame, callback) {
                async.each(frame.maths, function(math, callback) {
                    mathjax.typeset({
                        math: math.value,
                        format: "inline-TeX",
                        mml: true
                        ...
                        if (!data.errors) {
                            ...
                            if(math.type == constants.math_type_inline) {
                                frame.content = frame.content.replace(marker, constants.temp_inlineMath(data.mml));
                            } else if(math.type == constants.math_type_block) {
                                frame.content = frame.content.replace(marker, constants.temp_blockMath(data.mml));
                            }
    ...
    function translate(source, callback) {
        ...
        async.waterfall([
            ...
            function(slides, callback) {
                renderMathJax(slides, callback);
            }
        ], function(err, res) {

            if(err == null) {
                var html = "";
                res.frames.map(function (frame) {
                    frame.content = constants.temp_slide(frame.content, frame.properties);
                    html += frame.content;
                });
                res.html = ((res.titleSlide == undefined) ? "" : res.titleSlide) + html;
            }
            callback(err, res);
        });
    }
    ...
    if(process.env.Test == 1) {
        ...
        Translator.renderKatex = renderKatex;
        Translator.renderMathJax = renderMathJax;
    }
