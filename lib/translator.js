...
;(function(){
     ...
     * @returns {string} frame with restored math equations surrounded by either inline or block string (tags)
     */
    function renderKatex(frame, maths) {
        for(var i=0; i<maths.length; i++) {
            if(maths[i].type == constants.math_type_inline) {
                frame = frame.replace(constants.math_marker(i), katex.renderToString(maths[i].value));
            } else if(maths[i].type == constants.math_type_block) {
                frame = frame.replace(constants.math_marker(i), katex.renderToString("\\displaystyle {" + maths[i].value  + "}"));
            }
        }
        return frame;
    }
    ...
    function translate(source) {
        ...
        for(var i=0; i<frames.length; i++) {
            ...
            var newFrame = renderKatex(renderMarkdown(frameMath.frame), frameMath.maths);
